...
#    carbon_budget -= 2*e_0
    ...
    if "ex" in o:
        ...
        #force net-zero emissions in 2050
        CO2_CAP.loc[planning_horizons[-1], o]=0

    CO2_CAP.to_csv(path_cb + 'carbon_budget_distribution' + o + '.csv', sep=',',
                   line_terminator='\n',  float_format='%.3f')
...
def add_generation(network):
    ...
    for generator,carrier in [("nuclear","uranium")]:
        network.madd("Link",
                     nodes + " " + generator,
                     bus0=["EU " + carrier]*len(nodes),
                     bus1=nodes,
                     ...
                     efficiency=costs.at[generator,'efficiency'],
                     lifetime=costs.at[generator,'lifetime'])
    for generator,carrier in [("CCGT","gas")]:
        network.madd("Link",
                     nodes + " " + generator,
                     bus0=["EU " + carrier]*len(nodes),
                     bus1=nodes,
    ...
    for generator,carrier in [("coal","coal")]:
        network.madd("Link",
                     nodes + " " + generator,
                     bus0=["EU " + carrier]*len(nodes),
                     bus1=nodes,
...
if __name__ == "__main__":
    ...
    if 'snakemake' not in globals():
        ...
        snakemake = MockSnakemake(
            wildcards=dict(network='elec', simpl='370', clusters='37m', lv='1.0',
                           opts='', planning_horizons='2020',
                           sector_opts='3H-T-H-B-I-onwind+p3-dist1-cb27ex0'),
            ...
            output=['results/version-baseline/prenetworks/elec_s{simpl}_{clusters}_lv{lv}__{sector_opts}_{planning_horizons}.nc']
        )
        import yaml
    ...
    for o in opts:
        ...
        if "cb" in o:
            ...
            if not os.path.exists(path_cb):
                os.makedirs(path_cb)
            try:
                CO2_CAP=pd.read_csv(path_cb + 'carbon_budget_distribution' + o + '.csv', index_col=0)
            except:
    ...
    for o in opts:
        if "+" in o:
            ...
            if oo[0] == 'costofcoal':
                factor = float(oo[1][1:])
                n.generators.loc[n.generators.carrier.str.contains("lignite"),'marginal_cost'] *=factor
                n.generators.loc[n.generators.carrier.str.contains("coal"),'marginal_cost'] *=factor
                print("changing", " cost of coal and lignite ","by factor",factor)
