...
#What is the point of the 3code
#To draw a bound
Europe_ia3 = {  
            'Albania':'ALB',
            'Austria':'AUT',
...
# for key in Europe_ia3.keys():
#     if "shapefiles/gadm36_"+ Europe_ia3[key] + "_1.shp" in glob.glob("shapefiles/*"):
#         pass
#     else:
#         print ( Europe_ia3[key])
...
# gen_latdf = pd.read_csv('../data/0.25_gen_and_lat_no_sectors_no_transmission.csv', index_col = 0)
# gen_latdf = pd.read_csv('../data/0.25_gen_and_lat_no_sectors_yes_transmission.csv', index_col = 0)
gen_latdf = pd.read_csv('../data/0.25_gen_and_lat_yes_sectors_no_transmission.csv', index_col = 0) #For this one, we want to include the color bar
# gen_latdf = pd.read_csv('../data/0.25_gen_and_lat_yes_sectors_yes_transmission.csv', index_col = 0)
gen_latdf = gen_latdf[gen_latdf['carrier'] == 'solar']
gen_latdf.set_index('FullName', inplace = True)
...
for c in Europe_ia3.keys():
    if c in ['Bosnia_Herzegovina', "Macedonia"]:
        ...
        CC_plot = country_coord.loc[cdict[c]]
        # gas_abs = gas_df.loc[cdict[c]].T['gas_' + case + '_2019_TWh'] #This line is the amount of gas
...
#I do not think I need this section
#%% Pie chart - import
...
fig.savefig('../figures/Adamyessecnotrans.png',format='png',bbox_inches='tight')  
