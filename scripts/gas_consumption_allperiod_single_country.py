# -*- coding: utf-8 -*-
"""
Created on Fri Jun 10 12:39:09 2022
...
# important_countries = ['DE','GB','NL','IT','HU']
country = 'ES'
years = ['2020','2025','2030','2035','2040','2045','2050']
array_annual_gas = np.zeros([len(years)*2])

fig,axes = plt.subplots(nrows=len(years),ncols=2,figsize=[15,16],sharey='row')
# s_i = 0
axes_f = axes.flatten()
count = 0
for year in years:
    ...
    for scen in scens:
        ...
        array_annual_gas[count] = df_gas_t.sum().sum()/1e3
        ...
        ax.set_ylabel('GWh')
        ax.set_title(year + ' ' + scen_dic[scen],fontsize=fs)
        ax.set_ylim([0,ax.get_ylim()[1]])
        if scen[-2:] == 'gc':
            annual_reduction = np.abs(np.round(array_annual_gas[count] - array_annual_gas[count-1])) # in TWh
            annual_reduction_per = (array_annual_gas[count] - array_annual_gas[count-1])/array_annual_gas[count-1]*100 # in percentage
            
            annual_reduction = 0 if int(np.round(annual_reduction_per)) == 0 else annual_reduction
            
            mp = '' if annual_reduction_per < 0 else '+'
            ax.text(0.35,0.5,mp + str(int(np.round(annual_reduction_per))) + '% (' + str(int(annual_reduction)) + ' TWh)',fontsize=fs,transform=ax.transAxes,color='gray')
        
        count += 1 

legend_without_duplicate_labels(fig,axes_f)
...
fig.suptitle(country + ' ' + temp + 'C',fontsize=1.5*fs,y=0.925)

fig.savefig('../figures/Gas_consumption_' + country + '_' + temp + 'C.pdf', bbox_inches="tight")