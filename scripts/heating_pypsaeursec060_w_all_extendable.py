# -*- coding: utf-8 -*-
"""
Created on Fri Apr  1 10:19:18 2022
...
tech_colors['biomass boiler'] = tech_colors['residential rural biomass boiler']
# lngopt = '-LNG'
lngopt = ''
n_header = 4

b_df = pd.read_csv(
        '../results/PyPSA-Eur-Sec-0.6.0/3H_oil_lignite_nonextendable/csvs/supply_energy.csv',
        index_col=list(range(3)),
        header=list(range(n_header))
    )

c_df = pd.read_csv(
        '../results/PyPSA-Eur-Sec-0.6.0/3H_oil_lignite_nonextendable/csvs/capacities.csv',
        index_col=list(range(2)),
        header=list(range(n_header))
    )

tres = '3'
b_base_1p5C = b_df.T.loc[('37', '1.0', tres + 'H-T-H-B-I-A-solar+p3-dist1-cb25.7ex0')].T
b_nogas_1p5C = b_df.T.loc[('37', '1.0', tres + 'H-T-H-B-I-A-solar+p3-dist1-cb25.7ex0-gasconstrained' + lngopt)].T
b_base_2C = b_df.T.loc[('37', '1.0', tres + 'H-T-H-B-I-A-solar+p3-dist1-cb73.9ex0')].T
b_nogas_2C = b_df.T.loc[('37', '1.0', tres + 'H-T-H-B-I-A-solar+p3-dist1-cb73.9ex0-gasconstrained' + lngopt)].T

c_base_1p5C = c_df.T.loc[('37', '1.0', tres + 'H-T-H-B-I-A-solar+p3-dist1-cb25.7ex0')].T
c_nogas_1p5C = c_df.T.loc[('37', '1.0', tres + 'H-T-H-B-I-A-solar+p3-dist1-cb25.7ex0-gasconstrained' + lngopt)].T
c_base_2C = c_df.T.loc[('37', '1.0', tres + 'H-T-H-B-I-A-solar+p3-dist1-cb73.9ex0')].T
c_nogas_2C = c_df.T.loc[('37', '1.0', tres + 'H-T-H-B-I-A-solar+p3-dist1-cb73.9ex0-gasconstrained' + lngopt)].T
...
services_rural_heat_agriculture = b_df.loc[('services rural heat','loads','agriculture heat')]
...
heat_demand_dic = {'1.5C base':total_heat_demand.loc[('37', '1.0', tres + 'H-T-H-B-I-A-solar+p3-dist1-cb25.7ex0')],
                   '1.5C gas limit':total_heat_demand.loc[('37', '1.0', tres + 'H-T-H-B-I-A-solar+p3-dist1-cb25.7ex0')],
                   '2C base':total_heat_demand.loc[('37', '1.0', tres + 'H-T-H-B-I-A-solar+p3-dist1-cb25.7ex0')],
                   '2C gas limit':total_heat_demand.loc[('37', '1.0', tres + 'H-T-H-B-I-A-solar+p3-dist1-cb25.7ex0')]}
#%% Heating production INDEX
residential_heaters = [] # residential decentral heating 
services_heaters = [] # services decentral heating
...
for i in list(heating_dict.keys()):
    ...
    for k in range(len(heaters)):
        ...
        elif 'biomass CHP' in heaters[k][2]:
        ...
        elif 'biomass boiler' in heaters[k][2]:
            biomass_decentral.append(heaters[k])
#%% By source
sum_2020 = np.zeros(4)
sum_2025 = np.zeros(4)
sum_2030 = np.zeros(4)
...
for i in range(len(e_dict.keys())):
    ...
    leg_biomass = ax_i.bar(0,hist_wood_2019,bottom=hist_hp_2019 + hist_gas_2019 + hist_oil_2019+hist_coal_2019+hist_electricity_2019,color=color_dict2['Biomass'],width=wi)
...
ax[5].text(3,900,'Baseline',fontsize=fs)
ax[5].text(1.2,710,'Gas limit',fontsize=fs)

fig.legend(list(leg.values()) + [leg_coal],
           list(lab.values()) + ['Coal boilers'], 
           bbox_to_anchor=(0.85, 0.04),borderaxespad=0,prop={'size':fs},ncol=3)

fig.subplots_adjust(hspace=0.05)
fig.subplots_adjust(wspace=0.05)

fig.savefig('../figures/Heating_by_source_pypsaeursec060_oil_lignite_nonextendable' + lngopt + '.pdf', bbox_inches="tight")
...
def plot_series(network, carrier="heat", name="test"):
    ...
    (supply[new_columns].loc[start:stop]
    ...
    fig.tight_layout()
    #ax.plot(-supply['heat demand'],color=tech_colors['heat demand'],lw=10,zorder=100) This actually does not plot it
    plt.show()
...
n = pypsa.Network('../results/PyPSA-Eur-Sec-0.6.0/3H_oil_lignite_nonextendable/postnetworks/elec_s370_37_lv1.0__3H-T-H-B-I-A-solar+p3-dist1-cb25.7ex0-gasconstrained_2025.nc')
