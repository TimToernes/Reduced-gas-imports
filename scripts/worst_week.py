...
n15 = pypsa.Network()
#this is 1.5C, gas constrained, 2030
n15.import_from_netcdf("../results/PyPSA-Eur-Sec-0.6.0/Nuclear_decommission_years_biomass_boiler/postnetworks/elec_s370_37_lv1.0__3H-T-H-B-I-A-solar+p3-dist1-cb25.7ex0-gasconstrained_2030.nc")

#2C, gas constrained, 2030
n2 = pypsa.Network()
n2.import_from_netcdf("../results/PyPSA-Eur-Sec-0.6.0/Nuclear_decommission_years_biomass_boiler/postnetworks/elec_s370_37_lv1.0__3H-T-H-B-I-A-solar+p3-dist1-cb73.9ex0-gasconstrained_2030.nc")

n = pypsa.Network()
#2C, not gas constrained, 2030
n.import_from_netcdf("../results/PyPSA-Eur-Sec-0.6.0/Nuclear_decommission_years_biomass_boiler/postnetworks/elec_s370_37_lv1.0__3H-T-H-B-I-A-solar+p3-dist1-cb73.9ex0_2030.nc")



#

#esgen = n.generators.query('"bus 0" == "ES0 0"')
...
def make_plot_Spain():
    ...
    ESDemand = pd.read_csv("../results/PyPSA-Eur-Sec-0.6.0/Nuclear_decommission_years_biomass_boiler/csvs/Spain_demand.csv")#This is the same thing as ES0 0 loads_t.p (plus ES3 0)
...
def plot_series(network, ax, carrier="heat", name="test"):
    #To test 
...
def plot_series_elec(network, ax, carrier="heat", name="test"):
    ...
    supply = pd.DataFrame(index=n.snapshots)
    for c in n.iterate_components(n.branch_components):#for the lines, links, transformers
        n_port = 3 if c.name=='Link' else 1# what is n_port#Changing to 3 and 1 from 4 and 2
        for i in range(n_port):#can either be 4 or 2
            supply = pd.concat((supply,#concats from previous #looking at p0, p1, p2, etc
                                (-1) * c.pnl["p" + str(i+1)].loc[:,#looking at columns
                                                               c.df.index[c.df["bus" + str(i+1)].isin(buses)]].groupby(c.df.carrier,#looking at bus0, bus1, etc
                                                                                                                     axis=1).sum()),#what is pnl
                               axis=1)
...
fig, ax = plt.subplots()
plot_series_elec(n2, ax, carrier = 'AC')
...
if __name__ == "__main__":
    ...
    plot_series_elec(n, ax[0], carrier = 'AC')
    plot_series_elec(n2, ax[1], carrier = 'AC')

    plot_series(n, ax[2], carrier = 'heat')
    plot_series(n2, ax[3], carrier = 'heat')
    ...
    fig.suptitle(r"$\bf{2^\circ C \; scenarios\; in\; 2030}$", fontweight = 'bold', fontsize = fs)
    
    #This needs to stay
    fig.tight_layout(rect = [0, 0.25, 1, 1])


    fig.savefig("../figures/aggregate_heat_and_elec_2CCompare9.pdf")
